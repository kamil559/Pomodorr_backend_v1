...
from pomodorr.tools.permissions import IsObjectOwner, IsNotAuthenticated, IsTaskOwner
...
def test_is_task_project_owner_successfully(active_user):
    mock_request = Mock()
    mock_obj = Mock()
    mock_view = Mock()
...
def test_is_task_project_owner_with_different_user(active_user):
    mock_request = Mock()
    mock_obj = Mock()
    mock_view = Mock()
    obj_owner = Mock()
